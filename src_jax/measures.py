...
class GaussianMeasure(factors.ConjugateFactor):
    ...
    def _expectation_xAxx(self, A_mat: jnp.ndarray) -> jnp.ndarray:
        ...
        Exx = self._expectation_general_quadratic_inner(
            jnp.eye(self.D), jnp.zeros(self.D), jnp.eye(self.D), jnp.zeros(self.D)
        )
    ...
    def _expectation_xxTxxT(self) -> jnp.ndarray:
        ...
        return (
            2.0 * Sigma_mumu_p2
            + mumu_inner[:, None, None] * Sigma_mumu_m
            + Sigma_trace[:, None, None] * Sigma_mumu_p
        )
    ...
    def _expectation_xxTAxxT(self, A_mat: jnp.ndarray) -> jnp.ndarray:
        ...
        Sigma_mumu_m = self.Sigma - mumu_outer
        AAT = A_mat + A_mat.T
        Sigma_AA_Sigma = jnp.einsum(
            "abc, acd->abd", jnp.einsum("abc,cd->abd", Sigma_mumu_p, AAT), Sigma_mumu_p
        )
        muAmu = jnp.einsum(
            "ab,ab->a", self.mu, jnp.einsum("ab,ca-> cb", A_mat, self.mu)
        )
        ASigma_trace = self.get_trace(jnp.einsum("ab,cbd->cad", A_mat, self.Sigma))
        return (
            Sigma_AA_Sigma
            + muAmu[:, None, None] * Sigma_mumu_m
            + ASigma_trace[:, None, None] * Sigma_mumu_p
        )
