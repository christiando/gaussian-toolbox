...
class StateSpaceEM:
    ...
    def forward_path(self):
        ...
        Sigma_f, mu_f, Lambda_f, ln_det_Sigma_f = np.array(self.filter_density.Sigma), \
        ...
        self.prediction_density = densities.GaussianDensity(jnp.asarray(Sigma_p), jnp.asarray(mu_p),
                                                            jnp.asarray(Lambda_p), jnp.asarray(ln_det_Sigma_p))
        self.filter_density = densities.GaussianDensity(jnp.asarray(Sigma_f), jnp.asarray(mu_f),
    ...
    def backward_step(self, t: int, ps_dict: dict):
        ...
        cur_smoothing_density, cur_two_step_smoothing_density = self.sm.smoothing(cur_filter_density,
                                                                                  ...
                                                                                  uz_t=uz_t)
        cs_dict = {'Sigma': cur_smoothing_density.Sigma, 'mu': cur_smoothing_density.mu,
                   'Lambda': cur_smoothing_density.Lambda, 'ln_det_Sigma': cur_smoothing_density.ln_det_Sigma}
        ctss_dict = {'Sigma': cur_two_step_smoothing_density.Sigma, 'mu': cur_two_step_smoothing_density.mu,
    ...
    def backward_path(self):
        ...
        cs_dict = {'Sigma': last_filter_density.Sigma, 'mu': last_filter_density.mu,
                   'Lambda': last_filter_density.Lambda, 'ln_det_Sigma': last_filter_density.ln_det_Sigma}
        self.smoothing_density.update(jnp.array([self.T]), last_filter_density)

        Sigma_s, mu_s, Lambda_s, ln_det_Sigma_s = np.array(self.smoothing_density.Sigma), \
                                                  np.array(self.smoothing_density.mu), \
                                                  np.array(self.smoothing_density.Lambda), \
                                                  np.array(self.smoothing_density.ln_det_Sigma)
        Sigma_tss, mu_tss, Lambda_tss, ln_det_Sigma_tss = np.array(self.twostep_smoothing_density.Sigma), \
        ...
        self.smoothing_density = densities.GaussianDensity(Sigma=jnp.asarray(Sigma_s), mu=jnp.asarray(mu_s),
                                                           Lambda=jnp.asarray(Lambda_s),
                                                           ln_det_Sigma=jnp.asarray(ln_det_Sigma_s))
        self.twostep_smoothing_density = densities.GaussianDensity(Sigma=jnp.asarray(Sigma_tss), mu=jnp.asarray(mu_tss),
    ...
    def predict(self, X:jnp.ndarray, p0: 'GaussianDensity'=None, smoothed:bool=False, 
        ...
        for t in range(1, T+1):
            ...
            if u_x is not None:
                ...
                ux_t = None
            cur_filter_density = self.om.gappy_filtering(cur_prediction_density, X[t-1].reshape((1,-1)), ux_t=ux_t)
            ...
            if not smoothed:
                mu_ux, std_ux = self.om.gappy_data_density(cur_prediction_density, X[t-1].reshape((1,-1)), ux_t=ux_t)
                mu_unobserved[t-1, jnp.isnan(X[t-1])] = mu_ux
                std_unobserved[t-1, jnp.isnan(X[t-1])] = std_ux
            filter_density = densities.GaussianDensity(jnp.asarray(Sigma_f), jnp.asarray(mu_f),
                                                       jnp.asarray(Lambda_f), jnp.asarray(ln_det_Sigma_f))
        if not smoothed:
        ...
        # Smoothing
            ...
            for t in jnp.arange(T-1,-1,-1):
                ...
                if u_z is not None:
                    ...
                    uz_t = None
                cur_smoothing_density = self.sm.smoothing(cur_filter_density, cur_smoothing_density, uz_t=uz_t)[0]
                ...
                if u_x is not None:
                    ...
                    ux_t = None
                mu_ux, std_ux = self.om.gappy_data_density(cur_smoothing_density, X[t].reshape((1,-1)), ux_t=ux_t)
                mu_unobserved[t, jnp.isnan(X[t])] = mu_ux
                std_unobserved[t, jnp.isnan(X[t])] = std_ux
            smoothing_density = densities.GaussianDensity(Sigma=jnp.asarray(Sigma_s), mu=jnp.asarray(mu_s),
                                                          Lambda=jnp.asarray(Lambda_s),
                                                          ln_det_Sigma=jnp.asarray(ln_det_Sigma_s))
    ...
    def compute_data_density(self) -> densities.GaussianDensity:
        ...
        :return: GaussianDensity
            Data density.
        """
        px = self.om.emission_density.affine_marginal_transformation(self.prediction_density.slice(jnp.arange(1,self.T+1)))
        return px
