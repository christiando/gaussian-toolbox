...
class TestConjugateFactor:
    ...
    def test_init(self, R, D):
        ...
        nu = jnp.zeros((R, D))
        ln_beta = jnp.zeros(R)
        f = factors.ConjugateFactor(Lambda, nu, ln_beta)
    ...
    def test_slice(self, R, D, idx):
        Lambda = jnp.zeros((R, D, D))
        Lambda = Lambda.at[:].set(jnp.eye(D))
        Lambda = Lambda.at[idx].set(np.random.rand(len(idx))[:,None,None] * jnp.eye(D)[None])
        nu = jnp.zeros((R, D))
...
class TestOneRankFactor(TestConjugateFactor):
    ...
    def test_init(self, R, D):
        ...
        nu = jnp.zeros((R, D))
        ln_beta = jnp.zeros(R)
        f = factors.OneRankFactor(v, g, nu, ln_beta)
