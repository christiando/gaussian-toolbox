...
class TestGaussianMeasure(TestConjugateFactor):
    ...
    def test_multiply(self, R1, R2, D):
        ...
        Lambda1 = Lambda1.at[:].set(jnp.eye(D)[None] * np.random.rand(R1)[:,None,None])
        nu1 = jnp.zeros((R1, D))
        ln_beta1 = jnp.zeros(R1)
        m1 = self.test_class(Lambda1, nu1, ln_beta1)
        Lambda2 = jnp.zeros((R2, D, D))
        Lambda2 = Lambda2.at[:].set(jnp.eye(D)[None] * np.random.rand(R2)[:,None,None])
        nu2 = jnp.zeros((R2, D))
        ...
        nu_new = jnp.reshape(
            (m1.nu[:, None] + m2.nu[None]), (m1.R * m2.R, m2.D)
        )
