...
# Acknowldgements: Modified from chex https://github.com/deepmind/chex/blob/master/chex/_src/dataclass.py
# Enable kw_only argument.
...
def dataclass(
    ...
    """
    dcls = _Dataclass(
        init, repr, eq, order, unsafe_hash, frozen, mappable_dataclass, kw_only
    )
    if cls is None:
        return dcls
    return dcls(cls)
...
class _Dataclass:
    ...
    def __call__(self, cls):
        ...
        if invalid_fields:
            raise ValueError(
                f"The following dataclass fields are disallowed: "
