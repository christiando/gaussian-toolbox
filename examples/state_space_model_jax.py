...
def _generate_heteroscedastic_data(T, Dx, Dz, Du, sigma_z=0.02, sigma_x=0.02):
    params_dict = {
        "T": T,
        "Dx": Dx,
        "Dz": Dz,
    ...
    for idz in range(Dz):
        z[idz] = (
            1 * numpy.cos(2 * numpy.pi * numpy.arange(T) * freq[idz] + phase[idz])
            + noise_z[idz]
        )
    ...
    for t in range(T):
        Sigma_x = sigma_x ** 2 * numpy.eye(Dx) + numpy.dot(
            numpy.dot(U, numpy.diag(D_Sigma[:, t])), U.T
        )
...
def load_synthetic_data(
    Dz: int = 2, Dx: int = 7, Du: int = 3, T: int = 10000, sigma_x: float = 0.01, seed=1
):
...
if __name__ == "__main__":
    ...
    # %%
    from timeseries_jax import state_model
    from timeseries_jax import observation_model
    ...
    #     om = observation_model.HCCovObservationModel(Dx=dx, Dz=dz, Du=du)
    ...
    sm = state_model.LinearStateModel(dz)
    # om = observation_model.LinearObservationModel(dx, dz, noise_x=1.)
    om = observation_model.HCCovObservationModel(Dx=dx, Dz=dz, Du=du, noise_x=0.1)
    # om.pca_init(X, 50)
    ...
    ssm_em_lin = StateSpaceEM(
        jnp.array(X[:8000]), observation_model=om, state_model=sm, timeit=True
    )
    ssm_em_lin.run()
    # om = observation_model.HCCovObservationModel(Dx=dx, Dz=dz, Du=du)
    # om.pca_init(X, 50)
