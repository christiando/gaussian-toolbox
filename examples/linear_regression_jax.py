...
def generate_data(
    N: int, D: int, sigma_noise: float = 0.1, bias: bool = True
) -> Tuple[jnp.ndarray]:
...
if __name__ == "__main__":
    ...
    w_range, b_range = (
        jnp.linspace(w[0] - 0.2, w[0] + 0.2, 100),
        jnp.linspace(b[0] - 0.2, b[0] + 0.2, 100),
    )
    ...
    plt.plot(X, y, "k.")
    mu, std = (
        jnp.squeeze(prediction_density.mu),
        jnp.sqrt(jnp.squeeze(prediction_density.Sigma)),
    )
    plt.fill_between(jnp.squeeze(X_range), mu - std, mu + std, alpha=0.5)
