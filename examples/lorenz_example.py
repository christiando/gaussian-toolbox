...
X = observation_models.augment_taken(X, 5, 3)
# %%
Dx, Dz, Dk = X.shape[1], 3, 5
# Below is the only line, that changes!
sm = state_models.LSEMStateModel(Dz, Dk, noise_z=1.)
...
num_samples = 50
p_z_fo, mu_x_fo, std_x_fo = ssm_em.predict_static(jnp.array(X_test[:fully_observed_till]), observed_dims=jnp.arange(X_test.shape[1]))
p0 = p_z_fo.slice(jnp.array([-1]))
p_z_predict, mu_x_predict, std_x_predict = ssm_em.predict_static(jnp.array(X_test[fully_observed_till:]), p0=p0)
sample_z_predict, sample_x_predict = ssm_em.sample_trajectory_static(jnp.array(X_test[fully_observed_till:]), p0=p0, num_samples=num_samples)
...
for dx in range(Dx):
    ...
    plt.plot(sample_x[:,:3,dx], 'C3', alpha=.5)
    plt.plot(np.mean(sample_x[:,:,dx], axis=1), 'C3--', zorder=9)
    plt.fill_between(range(mu_predict.shape[0]), mu_predict[:,dx] - 1.68*std_predict[:,dx], mu_predict[:,dx] + 1.68*std_predict[:,dx], alpha=.5, zorder=9)
